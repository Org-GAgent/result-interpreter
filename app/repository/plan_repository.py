...
class PlanRepository:
    ...
    def are_children_complete(self, plan_id: int, parent_id: int) -> bool:
        """Check if all direct children of a node are in 'completed' status.
        
        Returns True if:
        1. The node has no children (is a leaf).
        2. All children have status='completed'.
        """
        ...
        with plan_db_connection(plan_path) as conn:
            # Check for any children existence first (optional optimization)
            # Or directly check for unfinished children.
            # If no children found, count is 0, logic holds if we check NOT IN 'completed'
            
            # Count children that are NOT completed
            # We treat NULL status as not completed.
            row = conn.execute(
                """
                SELECT COUNT(*) as unfinished_count 
                FROM tasks 
                WHERE parent_id = ? AND (status IS NULL OR status != 'completed')
                """,
                (parent_id,),
            ).fetchone()
            
            # If unfinished_count == 0, it means either:
            # a) No children exist (count=0) -> Returns True (Leaf node case)
            # b) Children exist but all are 'completed' -> Returns True
            return row["unfinished_count"] == 0
