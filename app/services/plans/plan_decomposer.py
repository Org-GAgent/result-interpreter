        ...
        prompt.extend(
            [
                ...
                *node_children,
                ...
                '         "meta": {',
                '             "<key>": "<value>",',
                '             "task_type": "code_required|text_only|data_summary (optional)"',
                '         }',
                "      }",
                "    }",
                "  ]",
                "}",
                "\n=== TASK TYPES ===",
                "You can optionally specify task_type in context.meta to control execution behavior:",
                "- code_required: Tasks requiring data processing, visualization, or computation (default for analysis tasks)",
                "- text_only: Pure text tasks like explanations, summaries, Q&A",
                "- data_summary: Generate text-based statistical analysis reports (NEW!)",
                "  * Use this for: 'generate data summary', 'statistical overview', 'data characteristics report'",
                "  * The system will intelligently analyze the dataset based on metadata and research context",
                "  * Produces targeted insights, not just basic mean/max/min statistics",
                "",
                "Example:",
                '  "meta": {"task_type": "data_summary"}  # for statistical summary tasks',
                "",
                "\nSTRICT REQUIREMENTS:",
                ...
                "- **IMPORTANT**: Specify `task_type` in `context.meta` for each task to control execution behavior:",
                '  * Use "data_summary" for tasks that generate statistical text reports',
                '  * Use "code_required" for tasks that create visualizations or perform computations',
                '  * Use "text_only" for pure text explanation tasks',
                '  * Example: "meta": {"task_type": "data_summary"}',
    ...
    def _process_queue(
        ...
        *,
        ...
        while queue and budget_remaining > 0:
            ...
            print(f"[run_plan] Decomposing node {current.node_id} at depth {current.relative_depth} (queue={len(queue)}, budget={budget_remaining})")
            ...
            for child in children:
                ...
                print(f"  -> Created task [{new_node.id}] {new_node.name}")
